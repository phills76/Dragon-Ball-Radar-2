...
// Initializing the Gemini API client using the environment variable API_KEY.
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

/**
 * Generates 7 dragon ball locations based on user location and search range using Gemini.
 */
export async function generateValidCoordinates(
  ...
  try {
    ...
    // Accessing .text as a property and providing a fallback for JSON parsing.
...
/**
 * Relocates a single dragon ball to a new public location.
 */
 ...
 */
function fallbackGeneration(userLoc: UserLocation, rangeKm: number): DragonBall[] {
